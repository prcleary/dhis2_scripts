cert="/etc/letsencrypt/live/dhis2.nih.org.pk/fullchain.pem"
echo " "
echo "############"
if sudo openssl x509 -checkend 86400 -noout -in $cert
then
  echo "The certificate $cert is good for at least another day!"
  today=`date +%D`
  expiredate=`sudo openssl x509 -enddate -noout -in $cert  | awk -F'=' '{print $2}'`
